...
class COVTestClientServices(Capability):
    ...
    def do_ConfirmedCOVNotificationRequest(self, apdu):
        ...
        assert hasattr(self, 'cov_ack')
        assert hasattr(self, 'cov_reject')
        assert hasattr(self, 'cov_abort')
        ...
        elif self.cov_reject:
            # reject
            response = RejectPDU(reason=self.cov_reject, context=apdu)
            if _debug: COVTestClientServices._debug("    - reject: %r", response)

        elif self.cov_abort:
            # abort
            response = AbortPDU(reason=self.cov_abort, context=apdu)
...
class TestBinaryValue(unittest.TestCase):
    ...
    def test_simple_transition(self):
        """Test basic configuration of a network."""
        if _debug: TestBinaryValue._debug("test_simple_transition")
        ...
        anet.td.cov_ack = True
        anet.td.cov_reject = None
        anet.td.cov_abort = None
